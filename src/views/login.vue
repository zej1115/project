							...
							<div class="form-content">
								...
								<div class="mg-b20 pd-r140 pos-relative">
									...
									<div class="w100 h100p pos-abs pos-c-r verification-code">
										<img @click="getVerificationCode" class="mouse-hand w100p h100p" :src="'data:image/png;base64,' + pngBase64" alt="verification code">
									</div>
...
<script>
	...
	export default {
		...
		methods: {
			checkLoginData() {
				let isTrue = true;
				if (trimString(this.loginData.userId)) {
				...
				if (trimString(this.loginData.password)) {
				...
				if (trimString(this.loginData.verificationCode)) {
			...
			toLogin() {
				...
				if (isTrue) {
					...
					window.sessionStorage.setItem('loginToken', '12321-21321-324324-45456');
					// this.$router.push({ path: '/'});
					this.$router.push({ path: '/select-system'}); // 登录成功则跳转至系统选择页
				}
			},
...
<style>
	.login-box-wrap { box-shadow:0px 2px 15px 0px rgba(0,0,0,0.14); }
